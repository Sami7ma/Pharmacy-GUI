...
public class HistoryTab {
    ...
    private void showSalesHistoryPopUp() {
        ...
        try (Statement stmt = DatabaseConnection.connection.createStatement();
            ...
            while (rs.next()) {
                ...
                row[2] = rs.getDate("SaleDate").toString();         // Sale Date (converted to String)
    ...
    private void showInventoryHistoryPopUp() {
        ...
        try (Statement stmt = DatabaseConnection.connection.createStatement();
            ...
            while (rs.next()) {
                ...
                row[4] = rs.getDate("ExpiryDate").toString();          // Expiry Date (converted to String)
    ...
    private void showPurchasesHistoryPopUp() {
        ...
        // Define columns for the JTable based on the Purchases table
        String[] columnNames = {"Purchase ID", "Supplier ID", "Purchase Date", "Total Amount"};
        
        // Create a list to store data rows
        ArrayList<String[]> dataList = new ArrayList<>();
        
        // Fetch the data from the Purchases table in the database
        String query = "SELECT PurchaseId, SupplierId, PurchaseDate, TotalAmount FROM Purchases";
        ...
        try (Statement stmt = DatabaseConnection.connection.createStatement();
            ...
            while (rs.next()) {
                ...
                row[1] = String.valueOf(rs.getInt("SupplierId"));   // Supplier ID
                row[2] = rs.getDate("PurchaseDate").toString();     // Purchase Date (converted to String)
                row[3] = String.valueOf(rs.getDouble("TotalAmount"));// Total Amount
